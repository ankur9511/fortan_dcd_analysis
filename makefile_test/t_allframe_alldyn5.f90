subroutine allfdyn_z(aid, t_t_outname, O_ID, HO_ID1, HO_ID2, OH_ID, &
        & HOH_ID1, HOH_ID2, &
        ...
        & dr_dt_2dmsd, dx_dt_2dmsd, dy_dt_2dmsd, n_dt_2dmsd, n_dt_RT

        real(4), dimension(0:nbinsdyn-1,0:maxdtdynsteps-1) :: dr2_dt_rc, K_dt_rc, &
        & dr2_dt_2dmsd, dx2_dt_2dmsd, dy2_dt_2dmsd, K_dt_2dmsd !, n_dt_RT
      ...
      & " = ",nbincrdf,"x",lensolv2drdfarr
        ...
        call cpu_time(T1)
        call initialize_K( K_dt_rc, &
        & K_dt_2dmsd, &
        & f_filename, outname, O_ID, dbindynwidth, dbincdynwidth, &
        & frame, aid, tot_frame, tot_atom, maxdtdynsteps, nbinsdyn, n_O_ID)
        !print *, "Outside initialize k"
        do m = frame, tot_frame - maxdtdynsteps*nskip - 3*nskip
        !print *, "At frame = ",m," , Last frame",tot_frame-maxdtdynsteps-10
        call alldyn_z(dr_dt_rc, n_dt_rc, dr2_dt_rc, K_dt_rc, &
       ...
       ! !!!!!!!!!!!!!!! End atom loop
        ...
        call cpu_time(T2)
       ! !print *, outname
        open(199, file=trim(outname)//"progressdyn.txt", &
...
!!
       ...
       ! !dr_dt_rc,n_dt_rc,dr_dt_2dmsd,n_dt_2dmsd,n_dt_RT
        ...
        dr2_dt_rc(i,j) = (dr2_dt_rc(i,j) - dr_dt_rc(i,j)*dr_dt_rc(i,j)/max(1.0,n_dt_rc(i,j)))/max(1.0,n_dt_rc(i,j))
        dr2_dt_2dmsd(i,j) = (dr2_dt_2dmsd(i,j) - dr_dt_2dmsd(i,j)*dr_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j)))/max(1.0,n_dt_2dmsd(i,j))
        dx2_dt_2dmsd(i,j) = (dx2_dt_2dmsd(i,j) - dx_dt_2dmsd(i,j)*dx_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j)))/max(1.0,n_dt_2dmsd(i,j))
        dy2_dt_2dmsd(i,j) = (dy2_dt_2dmsd(i,j) - dy_dt_2dmsd(i,j)*dy_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j)))/max(1.0,n_dt_2dmsd(i,j))

        dr_dt_rc(i,j) = K_dt_rc(i,j) + dr_dt_rc(i,j)/max(1.0,n_dt_rc(i,j))
        dr_dt_2dmsd(i,j) = K_dt_2dmsd(i,j) + dr_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j))
        dx_dt_2dmsd(i,j) = K_dt_2dmsd(i,j) + dx_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j))
        dy_dt_2dmsd(i,j) = K_dt_2dmsd(i,j) + dy_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j))
        end do
        dix = n_dt_RT(i,0)
        ...
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)

        open(2001, file=trim(outname)//"var_rc_dr.dat", access="stream")
        write(2001) dr2_dt_rc
        close(2001)

        open(200, file=trim(outname)//"var_rc_dr_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"var_rc_dr_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)
        ...
        open(2001, file=trim(outname)//"2dmsd_dx.dat", access="stream")
        write(2001) dx_dt_2dmsd
        close(2001)

        open(200, file=trim(outname)//"2dmsd_dx_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"2dmsd_dx_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)

        open(2001, file=trim(outname)//"2dmsd_dy.dat", access="stream")
        write(2001) dy_dt_2dmsd
        close(2001)

        open(200, file=trim(outname)//"2dmsd_dy_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"2dmsd_dy_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)

        open(2001, file=trim(outname)//"var_2dmsd_dr.dat", access="stream")
        write(2001) dr2_dt_2dmsd
        close(2001)

        open(200, file=trim(outname)//"var_2dmsd_dr_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"var_2dmsd_dr_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)

        open(2001, file=trim(outname)//"var_2dmsd_dx.dat", access="stream")
        write(2001) dx2_dt_2dmsd
        close(2001)

        open(200, file=trim(outname)//"var_2dmsd_dx_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"var_2dmsd_dx_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)

        open(2001, file=trim(outname)//"var_2dmsd_dy.dat", access="stream")
        write(2001) dy2_dt_2dmsd
        close(2001)

        open(200, file=trim(outname)//"var_2dmsd_dy_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nbinsdyn,maxdtdynsteps
        close(200)

        open(200, file=trim(outname)//"var_2dmsd_dy_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)
        ...
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz,dt,maxdtdynsteps*dt*nskip
        close(200)
end subroutine allfdyn_z

subroutine allfdyn(aid,t_t_outname,O_ID,HO_ID1,HO_ID2,OH_ID,HOH_ID1,HOH_ID2,Os_ID,subs_ID,filenames,dt,nfiles,frame,n_O_ID,n_OH_ID,n_Os_ID,n_subs_ID) 
        implicit none
        ...
        integer :: i, j, k, n, n1, ja, m, o, m1, tnpairs, &
        & maxdtdynsteps, nbinsdyn, ldist=50, nskip=1
        ...
        dbindynwidth = 5
        dbincdynwidth = 1 !5
        maxdtdynsteps = int(1000.0/dt/nskip)
        ...
        print *, SIZE(O_ID)," == ",n_O_ID
        print *, SIZE(OH_ID)," == ",n_OH_ID
        call allfdyn_z(aid, t_t_outname, O_ID, HO_ID1, HO_ID2, OH_ID, &
           & HOH_ID1, HOH_ID2, &
           & Os_ID, subs_ID, filenames, dt, frame, &
           & n_O_ID, n_OH_ID, n_Os_ID, n_subs_ID, &
           & nfiles, tot_frame, tot_atom, &
