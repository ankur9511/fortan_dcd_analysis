subroutine initialize_K( K_dt_rc, K_dt_2dmsd, &
                   & fname, outname, &
        ...
        real(4), intent(inout), dimension(0:nbins-1,0:maxdtsteps-1) :: K_dt_rc, &
                   & K_dt_2dmsd
        real(4),  dimension(0:nbins-1,0:maxdtsteps-1) :: n_dt_rc, &
                   & n_dt_2dmsd
        ...
        do m = frame+1, frame+1+maxdtsteps*nskip-3*nskip, nskip
         ...
         do i = 0,n_ID-1
           ...
           !dr2 = dr0t(0)*dr0t(0)
           !dx_dt_2dmsd(:,int((m-frame-1)/20.)) = dx_dt_2dmsd(:,int((m-frame-1)/20.))+dr2*flag(:,i)
           !dr2 = dr0t(1)*dr0t(1)
           !dy_dt_2dmsd(:,int((m-frame-1)/20.)) = dy_dt_2dmsd(:,int((m-frame-1)/20.))+dr2*flag(:,i)
           dr2 = dot_product(dr0t(0:1),dr0t(0:1))
           K_dt_2dmsd(:,int((m-frame-1)/fnskip)) = K_dt_2dmsd(:,int((m-frame-1)/fnskip))+dr2*flag(:,i)
           ...
           K_dt_rc(:,int((m-frame-1)/fnskip)) = K_dt_rc(:,int((m-frame-1)/fnskip)) + dr2*flag(:,i)
        ...
        do i = 0,nbins-1
        do j = 0,maxdtsteps-1
        K_dt_rc(i,j) = K_dt_rc(i,j)/max(1.0,n_dt_rc(i,j))
        K_dt_2dmsd(i,j) = K_dt_2dmsd(i,j)/max(1.0,n_dt_2dmsd(i,j))
        end do
        end do        
        !!!!!!!!! Residence Time !!!!!!!!!!!!!!!!!!!!
        !do i=0,nbins-1
