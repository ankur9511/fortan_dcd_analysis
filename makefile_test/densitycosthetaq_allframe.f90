...
subroutine denscosq_z(aid, t_t_outname, O_ID, HO_ID1, HO_ID2, OH_ID, &
        & HOH_ID1, HOH_ID2, &
...
!!
        ...
        open(2001, file=trim(outname)//"Okappaz.dat", access="stream")
        write(2001) Okappaz
        close(2001)

        open(200, file=trim(outname)//"Okappaz_dim.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) nz0
        close(200)

        open(200, file=trim(outname)//"Okappaz_range.dat",STATUS="REPLACE",ACTION="WRITE")
        write(200,FMT=*) -hbox0(2),-hbox0(2)+(nz0-1)*dz
        close(200)
...
end subroutine denscosq_z

subroutine denscosq(aid,t_t_outname,O_ID,HO_ID1,HO_ID2,OH_ID,HOH_ID1,HOH_ID2,Os_ID,subs_ID,filenames,dt,nfiles,frame,n_O_ID,n_OH_ID,n_Os_ID,n_subs_ID) 
        implicit none
        ...
        print *, SIZE(O_ID)," == ",n_O_ID
        print *, SIZE(OH_ID)," == ",n_OH_ID
        call denscosq_z(aid, t_t_outname, O_ID, HO_ID1, HO_ID2, OH_ID, &
           & HOH_ID1, HOH_ID2, &
           & Os_ID, subs_ID, filenames, dt, frame, &
           & n_O_ID, n_OH_ID, n_Os_ID, n_subs_ID, &
           & nfiles, tot_frame, tot_atom, &
