!!!! Purpose: Do all dynamic calculations
!!!! 2D MSD, Rotational Correlation, Residence time, Transition matrix/Permeability
!subroutine rotatecorr(dr_dt_rc, n_dt_rc, dr2_dt_rc, K_dt_rc)
!        implicit none
...
!end subroutine rotatecorr

!subroutine planemsd(dr_dt_2dmsd, dx_dt_2dmsd, dy_dt_2dmsd, &
!                    & dr2_dt_2dmsd, dx2_dt_2dmsd, dy2_dt_2dmsd, &
!                    & n_dt_2dmsd, K_dt_2dmsd)
!        implicit none
!        dr0t(:) = rt(ID(i)-1,:) - r0(ID(i)-1,:)
!
!        dr2 = dr0t(0)*dr0t(0)
!        dx_dt_2dmsd(:,lagt) = dx_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!        dx2_dt_2dmsd(:,lagt) = dx2_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!
!        dr2 = dr0t(1)*dr0t(1)
!        dy_dt_2dmsd(:,lagt) = dy_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!        dy2_dt_2dmsd(:,lagt) = dy2_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!
!        dr2 = dot_product(dr0t(0:1),dr0t(0:1))
!        dr_dt_2dmsd(:,lagt) = dr_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!        dr2_dt_2dmsd(:,lagt) = dr2_dt_2dmsd(:,lagt)+(dr2-K_dt_2dmsd(:,lagt))*(dr2-K_dt_2dmsd(:,lagt))*flag(:,i)
!
!        n_dt_2dmsd(:,lagt) = n_dt_2dmsd(:,lagt)+1.0*flag(:,i)
!end subroutine planemsd

!subroutine residencetime(n_dt_rt)
!        implicit none
!end subroutine residencetime

!subroutine transitionmat(Trans_zz)
!        implicit none
        !!! flag captures the residence of atom in a layer
        !!! trans_zz has to capture memory dependent transition
        !!! First map what all bins the atoms visits?
        !!! 
!end subroutine transitionmat
...
subroutine alldyn_z( dr_dt_rc, n_dt_rc, &
        ...
        integer, dimension(0:nbins-1,0:nbins-1) :: trans_mat_zz

        real(4), dimension(0:nbins-1) :: z0, zt, valz
        ...
        nreplica = int(dbinwidth/dbincwidth)
        ...
        do m = frame+1, frame+1 + (maxdtsteps-3)*nskip !-3*nskip, 1 !nskip
         !print *, m
         rt = 0.0         
